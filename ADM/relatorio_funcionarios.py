...
def criar_janela():
    # Criar a janela Toplevel para o Relatório de Funcionários
    janela = tk.Toplevel()
    janela.title("Relatório de Funcionários")
...
# Exemplo de como chamar a função para abrir a janela
# Aqui assumimos que você já tenha uma janela principal, ou pode criar um `root` para iniciar a aplicação.
if __name__ == "__main__":
    root = tk.Tk()
    root.title("Tela Principal")
    btn_open_report = tk.Button(root, text="Abrir Relatório de Funcionários", command=criar_janela)
    btn_open_report.pack(pady=20)
    root.mainloop()

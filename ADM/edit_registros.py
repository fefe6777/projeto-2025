...
def tela_registros():
    janela = tk.Toplevel()
    janela.title("Gerenciamento de registros")
    janela.configure(padx=20, pady=20)

    labels = ["IDR:", "ID:", "Hora:", "Imagem:", "Longitude:", "Latitude:", "Ultimo Registro:"]
    entradas = {}
    ...
    for campo in ["IDR:", "ID:", "Hora:", "Imagem:", "Longitude:", "Latitude:", "Ultimo Registro:"]:
    ...
    def consultar():
        ...
        try:
            ...
            if response.status_code == 200:
                dados = response.json()
                if dados:
                    entradas["Hora:"].config(state="normal")
                    entradas["Imagem:"].config(state="normal")
                    entradas["Latitude:"].config(state="normal")
                    entradas["Longitude:"].config(state="normal")
                    entradas["Ultimo registro:"].config(state="normal")
                    
                    entradas["Hora:"].delete(0, tk.END)
                    entradas["Hora:"].insert(0, dados["hora"])
                    entradas["Imagem:"].delete(0, tk.END)
                    entradas["Imagem:"].insert(0, dados["imagem"])
                    entradas["Latitude:"].delete(0, tk.END)
                    entradas["Latitude:"].insert(0, dados["latitude"])
                    entradas["Longitude"].delete(0, tk.END)
                    entradas["Longitude"].insert(0, dados["longitude"])
                    entradas["Ultimo registro:"].delete(0, tk.END)
                    entradas["Ultimo registro:"].insert(0, dados["ultimo_registro"])
